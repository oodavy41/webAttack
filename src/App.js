...
attacks = attacks.map((e) => {
  ...
  return { ...e, time: new Date(date).toLocaleString() };
});
...
export default class App extends Component {
  ...
  componentDidMount() {
    ...
    // resource ================================================= render
    let widthHeightRate = window.innerWidth / window.innerHeight;
    ...
    renderer.domElement.addEventListener("mouseup", this.rotation.onMouseUp);
  ...
  render() {
    ...
    return (
      <div
        ref={(m) => (this.div = m)}
        style={{ backgroundImage: `url(${space1Map})` }}
      >
        ...
        <div className={`${style.panel}`}>
          <div className={`${style.cell}`}>
            <div className={`${style.title}`}>当日恶意代码攻击事件数</div>
            <div className={`${style.count}`}>{attacks.length}个</div>
          </div>
          <div
            className={`${style.cell}  ${style.subCell} ${style.defSuccess}`}
          >
            <div className={`${style.subTitle}`}>当日攻击事件防御成功率</div>
          ...
          <div className={`${style.cell}`}>
            <table className={`${style.table}`}>
              {attacks.map((e, i) => (
                <tr
                  key={e.type + e.poster + e.aim}
                  onClick={() => {
                    this.setState({ pickedNode: attacks[i] });
                    this.rotation.focus(this.curves[i], () =>
                      this.setState({ pickedNode: null })
                    );
                  }}
                >
                  <td className={`${style.tableTd}`}>
                    <div className={`${style.attack_type}`}> {e.time}</div>
                    <div className={`${style.attack_type}`}> {e.type}</div>
